...
class InvertedResidual_modified(nn.Module):
    """
    ...
    def __init__(
        ...
        hidden_dim = make_divisible(int(round(in_channels // expand_ratio)), 16)

        super().__init__()

        block = nn.Sequential()

        if expand_ratio == 2 or expand_ratio == 4 or in_channels==out_channels:
            block.add_module(
                name="conv_3x3",
                module=ConvLayer(
                    in_channels=in_channels,
...
class MobileViT(nn.Module):
    ...
    def __init__(self, model_cfg: Dict, num_classes: int = 1000):
        ...
        # 测试添加注意力机制
        self.att = attention_block[2](exp_channels)
