...
def download():
    for i in range(len(kws)):
        # 使用爬虫下载图片
        pic_spider(kws[i], 5, 10)  # 自己设置多少页到多少页
        # 一边下载一边保存
        # 图片要保存的目的目录
...
def classfication():
    ...
    post_class_fication_count = {}          # 统计post_class_fication-当前类别的总数目
    post_class_fication_false_count = {}    # 统计post_class_fication-当前类别的分类错误数目
    for kw in kws:
    ...
    # 统计准确度：
    for kw in kws:
        print("统计{}-当前类别的数目：{}".format(kw, post_class_fication_count[kw]))
        print("统计{}-当前类别的分类错误数目：{}".format(kw, post_class_fication_false_count))
        # 保留6为小数位
        accuracy = format(1 - (post_class_fication_false_count[kw] / post_class_fication_count[kw]), '%.6f')
        print("{}-当前类别的分类准确率：{}\n".format(kw, accuracy))
